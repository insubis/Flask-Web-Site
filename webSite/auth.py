...
def login():
    if request.method == 'POST':
        ...
        if user:
            ...
            flash('Email não exsite.', category='error')
...
def sign_up():
    if request.method == 'POST':
        ...
        if user:
            ...
            flash('Email tem que ter mais de 4 caracteris.', category='error')
        elif len(first_name) < 2:
            flash('Nome muito Curto.', category='error')
        elif password1 != password2:
            flash('As senhas não correspondem.', category='error')
        elif len(password1) < 7:
            flash('Senha tem que ter mais de 7 caracteris.', category='error')
        else:
